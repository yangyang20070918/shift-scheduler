...
| Type | 内部值 | 说明 | 示例 | 自動排班 | 需要人数計算 | 個人労働時間 |
|------|--------|------|------|---------|-------------|-------------|
| 勤務 | work | 通常勤務 | 日勤、夜勤、早番 | ✅ 自動分配 | ✅ 計入 | ✅ 計入 |
| 休日 | rest | 休息日 | 休日 | ✅ 自動分配（一般休日のみ） | ❌ 除外 | ❌ 除外 |
| 出張 | travel | 出張 | 出張A、研修出張 | ❌ 固定割当のみ | ❌ 除外 | ✅ 計入 |

**各类型の業務ルール：**

- **勤務（work）**：メンバーの「対応可能パターン」に登録するのは勤務タイプのみ。自動排班時、ソルバーが各メンバーの可能パターンから最適に割当。
- **休日（rest）**：自動排班結果には一般的な「休日」のみが割り当てられる。特殊な休日（有給、祝日休等）は客户が固定割当で指定。同一タイプで複数の名前を付けることは可能（例：「休日」「有給休暇」）。
- **出張（travel）**：自動排班の対象外。管理者が固定割当で指定する。個人の労働時間・出勤日数には算入するが、毎日の需要人数（P9）やグループ需要（P8）には算入しない。出張は実労働時間の設定が必要。

**UI上の表示：**

| Type | 表示項目 |
|------|---------|
| 勤務 | 名前、開始時刻、終了時刻、実労働時間、休憩時間、色 |
| 休日 | 名前、色のみ |
| 出張 | 名前、実労働時間、色 |

好处：
- 统计时 `pattern.type == 'work'` 即可，不用判断具体的Pattern名称
- 不同Type有不同的计算规则（出張は需要に計入しない、休日は労働時間に計入しない）
- 同一Type内で複数の名前・設定を持てる（例：勤務タイプに日勤8h、夜勤10h）
...
### 12.3.1 出勤パターンExcel导出导入

- **导出**：当前已登录的全部出勤パターンをExcel出力（名前、タイプ、開始/終了時刻、休憩時間、実労働時間、色コード）
- **导入**：Excelから出勤パターンを一括読込。既存パターン名と一致する場合は更新、新規名は追加。プレビュー画面で新規/更新を確認後、実行。
...
### Phase 9：機能完善 — テスト後の追加開発

テスト期間中に特定された未実装機能および改善項目。

#### 9.1 出張タイプ対応（ソルバー）

出張パターンのソルバー内での特殊処理：
- PatternType に TRAVEL を追加
- P9（毎日需要）：出張パターンを需要人数にカウントしない
- P8（グループ需要）：出張パターンをグループ需要にカウントしない
- P2〜P7（個人制約）：出張は出勤日数・労働時間として計算する
- 自動排班対象外（固定割当でのみ指定可能）

#### 9.2 連続勤務禁止（P1-A）API + UI

DB モデル既存（forbidden_transitions テーブル）。API と UI を追加：
- **API**: GET/POST/DELETE /api/forbidden-transitions
- **UI**: 出勤パターン設定画面にマトリクス形式の禁止設定セクション追加
  - 行 = 前日のパターン、列 = 翌日のパターン
  - チェックで禁止ペアを設定

#### 9.3 パターンチェーン（P1-B）API + UI

DB モデル既存（pattern_chains テーブル）。API と UI を追加：
- **API**: GET/POST/PUT/DELETE /api/pattern-chains
- **UI**: 出勤パターン設定画面にチェーン定義セクション追加
  - トリガーパターン + 後続シーケンス（各ノードでパターン候補 or 休日を選択）
  - 例：夜勤 → [-] → [休] = 連鎖3日

#### 9.4 パターン別毎日需要

現状は毎日の総人数（min_total/max_total）のみ。パターン別の需要を追加：
- **DB**: pattern_demands テーブル新規作成（schedule_id, date, pattern_id, min_count）
- **API**: GET/POST/DELETE /api/schedules/{id}/pattern-demands + batch エンドポイント
- **UI**: スケジュール設定画面に「パターン別需要」セクション追加
  - 例：日勤 最小2人、夜勤 最小1人
- **ソルバー**: pattern_demands を SolverInput に含め、P9 で処理

#### 9.5 グループ需要一括設定

現状は1日ずつ設定。一括設定モードを追加：
- **API**: POST /api/schedules/{id}/group-demands/batch
- **UI**: グループ + パターン + 最小人数を選択 → 全日程に一括適用

### Phase 10：結果画面 統計ダッシュボード

排班結果画面（ScheduleResultPage）に、排班表の下にトグル可能な5つの統計セクションを追加する。
...
```
[日別統計] [パターン別需要] [グループ需要] [個人統計] [全体バランス]
```

各ボタンは ON/OFF 切替。デフォルトは全部 OFF（排班表のみ表示）。

#### 10.2 日別出勤統計

排班表と同じ日付列で、以下の行をテーブル表示：

| 行 | 内容 |
|----|------|
| 出勤人数 | その日の出勤者数（is_rest=false） |
| 必要人数 | 設定した需要 min〜max |
| 過不足 | 出勤人数 - min（マイナス=赤、0以上=緑） |

必要データ：`demands`（DailyDemand 一覧）

#### 10.3 パターン別需要充足

パターン需要が設定されているパターンごとに1行：

```
早番: 7/1: 2/2 ✓ | 7/2: 1/2 ✗ | 7/3: 3/2 ✓ | ...
遅番: 7/1: 1/1 ✓ | 7/2: 1/1 ✓ | 7/3: 0/1 ✗ | ...
```

- 分子 = その日にそのパターンで出勤した実人数
- 分母 = パターン別需要の min_count
- 充足 → 緑、不足 → 赤

必要データ：`patternDemands`（PatternDemand 一覧）

#### 10.4 グループ需要充足

グループ需要設定がある場合のみ表示。グループ×パターンの組み合わせごとに1行：

```
グループA(早番): 7/1: 1/1 ✓ | 7/2: 0/1 ✗ | ...
グループA(全体): 7/1: 3/2 ✓ | 7/2: 2/2 ✓ | ...
```

- 分子 = 実績人数（該当グループメンバーの出勤数）
- 分母 = 必要最小人数
- 充足 → 緑、不足 → 赤

必要データ：`groupDemands`（GroupDemand 一覧）、`groups`（Group 一覧 + member_ids）

#### 10.5 個人統計

メンバーごとの集計。排班表と同じメンバー行に対応する追加列：

| 列 | 計算方法 |
|----|---------|
| 出勤日数 | is_rest=false の日数 |
| 休日数 | is_rest=true の日数 |
| 労働時間 | 各出勤日のパターン work_hours の合計 |
| パターン別日数 | 早番:10日、遅番:5日 等（各パターンの日数） |

必要データ：パターンの `work_hours`（現在の patterns map に追加）
...
追加データ取得（結果ページの load 関数で追加ロード）：
- `listDemands(id)` → 日別需要
- `listPatternDemands(id)` → パターン別需要
- `listGroupDemands(id)` → グループ需要
- `listGroups()` → グループ情報（member_ids 含む）
- パターン map に `work_hours` を追加

### Phase 11：Excel/PDF統計出力 + 自動タスク

#### 11.1 Excel出力の統計強化

統計情報Sheetを5シート構成に拡充：
1. **排班表** — 排班グリッド（既存）
2. **日別需要充足**（新規） — 日別出勤統計 + パターン別需要充足 + グループ需要充足
3. **統計情報**（強化） — 健全スコア + メンバー別パターン日数 + 全体バランス（平均/最小/最大/標準偏差）
4. **違反レポート** — 既存
5. **警告** — 既存

エンドポイント: `GET /api/schedules/{id}/export/excel`（既存、内部強化）

#### 11.2 PDF出力

reportlabによるPDF生成。A4横置き（日数>15時）/ 縦置き。内容：
- 排班グリッド（色付き、パターン名表示）
- 日別出勤統計（出勤人数/必要人数/過不足）
- 個人統計テーブル（出勤日数/休日数/労働時間/パターン別日数）
- 全体バランス（平均/最小/最大/標準偏差）
- 違反レポート + 警告

エンドポイント: `GET /api/schedules/{id}/export/pdf`
フロントエンド: 結果画面に「PDF出力」ボタン追加

#### 11.3 休息希望自動締切処理

バックグラウンドタスク（1時間間隔）で以下を自動実行：
- `rest_request_deadline` が過ぎた `requesting` 状態のスケジュールを自動クローズ
- 未提出の休息希望を自動提出（is_auto_submitted=true）
- 提出済みの日付を固定割当（P0 rest）に変換

手動トリガー: `POST /api/tasks/auto-close-rest-requests`

#### 11.4 履歴データクリーンアップ

180日以上前のスケジュールのassignmentデータをクリア（メタ情報は保持）。

手動トリガー: `POST /api/tasks/cleanup-old-data`

#### 11.5 マルチテナント分離

全モデルに `TenantMixin`（tenant_id列）適用済み。全クエリでJWTから取得したtenant_idでフィルタリング。
検証は統合テストフェーズで実施。

### Phase 12：統合テスト + 品質検証

Phase 8の計画に基づき、以下のテストを実施・追加。

#### 12.1 テスト一覧

| テストファイル | 件数 | 内容 |
|--------------|------|------|
| test_solver_basic.py | 4 | Solver基本動作 |
| test_edge_cases.py | 6 | 境界ケース |
| test_explain_scenario.py | 2 | Explain Engine |
| test_api.py | 10 | API CRUD + 認証 + テナント分離 |
| test_api_integration.py | 18 | 統合テスト（需要/固定割当/制約/グループ/休息希望/テナント/セキュリティ） |
| test_phase11.py | 12 | Excel/PDF出力、パターン需要、自動締切タスク |
| test_performance.py | 4 | 10/20/50/100人規模の性能テスト |

合計: 56テスト（性能テスト除く70テスト通過確認済み）
...
テストで検証済みの項目：
- JWT認証（未認証/無効トークン/期限切れトークン → 401）
- テナント分離（メンバー/パターン/スケジュール/制約/休息希望で確認）
- 個人トークンのテナント境界（他テナントのスケジュールにアクセス不可）
- 二重提出防止（休息希望の重複submit → 400）
- 締切後の変更拒否（close後のsubmit → 400）
